...
\textbf{Anhand der vorliegenden Untersuchungen bezüglich Vollzeit- und Teilzeitstudierenden kann die \hyref{hypothese2} nicht belegt werden.}
...
\textbf{Die durchgeführten Analysen stützen, die in diesem Kapitel erforschte \hyref{hypothese1}.}

\newpage
\section{Pull Request Akzeptanz}
In diesem Abschnitt wird die \fref{forschungsfrage2} "\textit{Können die Schliessungsgründe der PRs in den Projekten ermittelt und klassifiziert werden?}" untersucht. Ziel ist es, die Gründe für die Ablehnung oder das Schliessen von PRs systematisch zu erfassen und mögliche Muster zwischen Vollzeit- und Teilzeitklassen herauszuarbeiten.
...
Um die \fref{forschungsfrage4} klären
...
\subsection{Korrelationsanalyse von OpenSource Projekten}
todo: einführung warum opensource repos, warum macht vergleich sinn
\subsubsection{Vorstellung der GitHub-Organisationen im Vergleich zu Racetrack-Projekten}
...
Zur besseren Einschätzung der Zusammenhänge zwischen den verschiedenen Metriken wird jeweils eine Korrelationsmatrix der verschiedenen GitHub-Organisationen erstellt. Die Analyse basiert auf der Spearman-Rangkorrelation. 

Die Korrelationsmatrix der Ubique Github Organisation ist in Abbildung \autoref{fig:korrelationsmatrix-ubique} dargestelltt. 

\begin{figure}[htbp]
    \includegraphics[width=\textwidth]{Figures/ubique-korrelationsmatrix.png}
    \caption{Korrelations-Heatmap der Metriken der Ubique Innovation AG}
    \label{fig:korrelationsmatrix-ubique}
\end{figure}


\autoref{fig:korrelationsmatrix-sbb} zeigt die Korrelationsmatrix der Metriken der Github Organisation der Schweizerischen Bundesbahn (SBB). 
\begin{figure}[htbp]
