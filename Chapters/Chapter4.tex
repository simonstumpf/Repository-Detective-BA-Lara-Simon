...
Requests (Zeit bis zum Merge oder Schliessung) und dem Churn (Anzahl geänderter Codezeilen) des jeweiligen Pull Requests?}" wurden die geminten\footnote{Abgeleitet vom englischen Verb to mine.} Repository Daten in einem Jupiter Notebook aufbereitet und graphisch dargestellt. Dazu wurden die im Kapitel \secref{sec:Metriken} erwähnten Metriken \textit{createTime} und \textit{closeTime} gemint und damit die einzelnen Latencies berechnet. Um die Latency-Daten besser vergleichen zu können, wurden diese noch vom Datumsformat in Stunden umgerechnet. Des Weiteren wurden die Metriken  \textit{additions} und  \textit{deleteions} aus den Pull Requests extrahiert, um den Churn zu berechnen.
...
Zur detaillierten Analyse der \fref{forschungsfrage5} "\textit{Welche Unterschiede zeigen sich in der Nutzung von Pull Requests zwischen Teilzeit- und Vollzeitstudierenden hinsichtlich Anzahl und Umfang?}" erfolgt eine Aufteilung der Repository-Daten auf Teilzeit- und Vollzeitklassen. Die Anzahl der Pull Requests der Teilzeitklassen beläuft sich auf 1369 und jene der Vollzeitklassen auf 1066. Dabei werden durchschnittlich 31.8 Pull Requests von den Teilzeitstudierenden erstellt und 39.5 von den Vollzeitstudierenden. In der folgenden Tabelle werden die wichtigsten Kennzahlen aufgezeigt.
...
In diesem Kapitel wird die \fref{forschungsfrage4} untersucht. Dazu werden zunächst die Metriken der Racetrack-Projekte analysiert und eine Korrelationsmatrix erstellt und anschliessend mit den OpenSource-Projekten der GitHub-Organisationen Ubique Innovation AG, Schweizerische Bundesbahnen (SBB) und Zalando SE verglichen.
...
Um die \fref{forschungsfrage1} im Kontext der Open-Source-Projekte zu untersuchen, werden Latency und Churn in den Repositories der drei GitHub-Organisationen analysiert. 

Die Tabelle \autoref{tab:latency-churn-open-source} zeigt die wichtigsten statistischen Kennzahlen für \textit{latency} und \textit{churn} getrennt nach Organisationen.

\begin{table}[htbp]
    \centering
    \caption{Kennzahlen zu \textit{Latency} und \textit{Churn} in Open-Source-Projekten}
    \label{tab:latency-churn-open-source}
    \begin{tabular}{@{}lrrrrrr@{}}
        \toprule
        \textbf{Organisation} & \textbf{Mittelwert} & \textbf{Median} & \textbf{Q1} & \textbf{Q3} & \textbf{Std.-abw.} & \textbf{Max} \\
        \midrule
        \multicolumn{7}{l}{\textit{Latency (in Stunden)}} \\
        Ubique & XX & XX & XX & XX & XX & XX \\
        SBB & XX & XX & XX & XX & XX & XX \\
        Zalando & XX & XX & XX & XX & XX & XX \\
        \addlinespace
        \multicolumn{7}{l}{\textit{Churn (Zeilenänderungen)}} \\
        Ubique & 720.3 & 11.5 & XX & XX & XX & XX \\
        SBB & 2042 & 41 & XX & XX & XX & XX \\
        Zalando & 282.1 & 4 & XX & XX & XX & XX \\
        \bottomrule
    \end{tabular}
\end{table}
...
Die Ergebnisse aus der \fref{forschungsfrage4} stellten sich als spannende Metrik heraus, für eine Erweiterung in GitGauge. Dabei soll angezeigt werden wann die Teammitglieder gearbeitet haben. Dies soll den Dozierende helfen, zu erkennen, ob es Probleme bei der Zusammenarbeit der Studierenden gibt.
\subsection{Erkennung Arbeitszeiten}
Die Arbeitszeiten sollten anhand der Commits in den Projekten erkannt werden. Dabei ist zu beachten, dass der Main Branch nicht alle Commits in den Projekten anzeigt, da Commits gesquashed werden können. Um jedoch an diese gesquashten Commits zu gelangen, werden zusätzlich die Pull Requests durchlaufen und deren Commits ausgelesen. Wenn jedoch Entwickler ihre Commits gesquashed haben, ohne dies über einen Pull Request zu tun, können die Commit-Daten nicht wiederhergestellt werden.
